...
describe('Basic Types', () => {
    test('Creates motto', () => {
      expect(LOG[0]).toEqual('UWF is my school');
    });
    test('Logs motto length', () => {
      expect(JSON.parse(LOG[1])).toEqual(16);
    });
    test('Finds "UWF" presence', () => {
      expect(JSON.parse(LOG[2])).toEqual(false);
    });
    test('Logs expanded motto', () => {
      expect(LOG[3]).toEqual('The University of West Florida is my school');
    });
    test('Calculates motto length ratio', () => {
      expect(LOG[4]).toEqual('268.75%');
    });
  });
